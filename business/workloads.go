...
func fetchWorkloads(layer *Layer, cluster, namespace string, labelSelector string) (models.Workloads, error) {
	...
	go func() {
		...
		if IsNamespaceCached(cluster, namespace) {
			...
			dep, err = remoteIstioClusters[cluster].K8s.GetDeployments(namespace)
	...
	// clonesetController := map[string]string{}
		...
		if len(pod.OwnerReferences) != 0 {
			for _, ref := range pod.OwnerReferences {
				if ref.Controller != nil && *ref.Controller {
					...
					// 	clonesetController[ref.Name] = ref.Kind
					// 	continue
					// }
	...
	for _, cname := range cnames {
		...
		case kubernetes.CloneSetType:
			...
			w.SetPods(kubernetes.FilterPodsForSelector(selector, pods))
			w.Name = cname
			w.Type = kubernetes.CloneSetType
