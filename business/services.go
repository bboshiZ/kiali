...
func (in *SvcService) GetServiceList(cluster, namespace string, linkIstioResources bool) (*models.ServiceList, error) {
	...
	go func() {
		...
		if IsNamespaceCached(cluster, namespace) {
			...
			svcs, err2 = kialiRemoteCache[cluster].GetServices(namespace, nil)
		} else {
	...
	go func() {
		...
		if IsNamespaceCached(cluster, namespace) {
			...
			pods, err2 = kialiRemoteCache[cluster].GetPods(namespace, "")
		} else {
	...
	go func() {
		...
		if IsNamespaceCached(cluster, namespace) {
			...
			deployments, err2 = kialiRemoteCache[cluster].GetDeployments(namespace)
		} else {
...
func (in *SvcService) buildServiceList(namespace models.Namespace, svcs []core_v1.Service, pods []core_v1.Pod, deployments []apps_v1.Deployment, linkedResources map[string]*[]kubernetes.IstioObject) *models.ServiceList {
	...
	for i, item := range svcs {
		...
		deployKind := ""
		if len(sPods) > 0 && len(sPods[0].OwnerReferences) > 0 {
			switch sPods[0].OwnerReferences[0].Kind {
			case "ReplicaSet":
...
func (in *SvcService) GetServiceDefinitionList(namespace string) (*models.ServiceDefinitionList, error) {
	...
	if IsNamespaceCached("", namespace) {
		...
		for _, remoteCluster := range remoteIstioClusters {
			s, err := remoteCluster.K8s.GetServices(namespace, nil)
			if err != nil {
				svcs = append(svcs, s...)
			}
		}
